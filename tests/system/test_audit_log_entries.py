...
def test_successful_login_creates_audit_entry(auth_client):
    ...
    login_entry = next(
        (e for e in entries if e["action"] == "api_login_successful"), None
    )
...
def test_failed_login_creates_audit_entry(auth_client):
    ...
    failed_entry = next((e for e in entries if e["action"] == "api_login_failed"), None)
...
def test_form_submission_creates_audit_entry(forms_client):
    ...
    # Verify audit log entry
    entries = read_audit_log(log_path)
    form_entry = next((e for e in entries if e["action"] == "api_form_submitted"), None)
...
def test_export_request_creates_audit_entry(exports_client):
    ...
    entries = read_audit_log(log_path)
    export_entry = next(
        (e for e in entries if e["action"] == "api_export_requested"), None
    )
