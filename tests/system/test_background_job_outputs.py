...
def test_submitted_form_persisted_to_json(forms_client):
    ...
    submitted = next(
        (s for s in submissions if s.get("name") == form_name),
        None
    )
...
def test_multiple_submissions_accumulate(forms_client):
    ...
    assert len(submissions) == len(submissions_to_make), \
        f"Expected {len(submissions_to_make)} submissions, got {len(submissions)}"
    
    for name, email, message in submissions_to_make:
        submitted = next(
            (s for s in submissions if s.get("name") == name),
            None
        )
...
def test_submission_has_required_fields(forms_client):
    ...
    # Submit form
    forms_client.submit_form("Test User", "test@example.com", "Test message content here")
...
def test_submission_timestamp_is_valid(forms_client):
    ...
    # Submit form
    forms_client.submit_form("Timestamp User", "timestamp@example.com", "Message for timestamp test")
    ...
    # ISO format check
    assert "T" in timestamp or ":" in timestamp, f"Invalid timestamp format: {timestamp}"
