...
def assert_schema_has_required_fields(
    response: requests.Response, fields: list
) -> None:
    """Assert response has all required fields."""
    body = response.json()
    missing = [f for f in fields if f not in body]
    assert not missing, f"Response missing required fields: {missing}. Got: {body}"
    logger.debug("Assertion passed: response has all required fields")
