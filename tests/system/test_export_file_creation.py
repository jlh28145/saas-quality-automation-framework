...
def test_export_file_has_csv_headers(exports_client):
    ...
    assert headers == expected_headers, f"Headers mismatch. Got: {headers}, Expected: {expected_headers}"
...
def test_export_file_contains_submitted_forms(forms_client, exports_client):
    ...
    submitted_row = next(
        (row for row in rows if row.get("name") == form_name),
        None
    )
    
    assert submitted_row is not None, f"Form submission not found in export. Rows: {rows}"
...
def test_export_response_matches_file_content(forms_client, exports_client):
    ...
    assert actual_record_count == response_record_count, \
