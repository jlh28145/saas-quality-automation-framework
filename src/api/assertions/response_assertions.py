...
def assert_json_body_contains(
    response: requests.Response, key: str, value: Any = None
) -> None:
    ...
    if value is not None:
        assert body[key] == value, f"Expected {key}={value}, got {key}={body[key]}"
    logger.debug(f"Assertion passed: {key} in response body")


def assert_json_field_equals(
    response: requests.Response, key: str, expected: Any
) -> None:
...
def assert_json_field_is_type(
    response: requests.Response, key: str, expected_type: type
) -> None:
